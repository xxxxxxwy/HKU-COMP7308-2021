...
def sample_motion_model(odometry, particles):
    ...
    '''your code here'''
    '''***        ***'''
    sigma_del_rot1 = noise[0] * abs(delta_rot1) + noise[1] * delta_trans
    sigma_del_rot2 = noise[0] * abs(delta_rot2) + noise[1] * delta_trans
    sigma_del_trans = noise[2] * delta_trans + noise[3] * (abs(delta_rot1) + abs(delta_rot2))

    for p in particles:
        n_del_rot1 = delta_rot1 + np.random.normal(0, sigma_del_rot1)
        n_del_rot2 = delta_rot2 + np.random.normal(0, sigma_del_rot2)
        n_del_trans = delta_trans + np.random.normal(0, sigma_del_trans)
    ...
    for particle in particles:
        ...
        for i in range(len(ids)):
            ...
            if not landmark['observed']:
                ...
                '''your code here'''
                '''***        ***'''
                mx = px + meas_range * np.cos(ptheta + meas_bearing)
                my = py + meas_range * np.sin(ptheta + meas_bearing)
                landmark['mu'] = [mx, my]
                ...
                Q = H.dot(sigma).dot(H.T) + Q_t
                K = sigma.dot(H.T).dot(np.linalg.inv(Q))
